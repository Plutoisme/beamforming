...
# we name the mic array as CMA100.
class CMA100():
    def __init__(self, num_angle=6, num_micarray=4, num_mic=5, r=[i for i in [0.008, 0.016, 0.032, 0.064]], 
    fs=32000, L=1024, c=340, decay_angle=120):
        ...
        for i in range(num_angle):
            ...
            for j in range(num_micarray):
                ...
                for k in range((num_mic-1)//2):
                    ...
                    mic_x.append((k+1)*(-ri) * np.cos(theta))
                    mic_y.append((k+1)*(-ri) * np.sin(theta))
                mic_z = [2.800 for num in range(len(mic_x))]
        ...
        # 设置基本频率以及 k值分配
        '''
        for example : fs = 32000, frameLength=1024, k_value=[512, 256, 128, 64, 32]
        '''
        self.k_value = []
    ...
    def set_signal(self, i, j, m, signal_rfft=None):
        ...
        4组阵元间距: [0.008, 0.016, 0.032, 0.064], 间距越短， 处理的频率越高
        513个频点对应的是0-16k的频率, (根据Nyquist采样定理, 一半的频率即可无失真重建目标频率信号)
        因此有
        阵元间距d=0.008对应的频段： 0-2k , 对应FFT的k值: [0, 64]   ->>>> 2^0 - 1 , 2^5
        d = 0.016对应的频段: 2k-4k 对应FFT的k值: [65, 128]   ->>>>   2^5 + 1 , 2^6
        d = 0.032对应的频段: 4k-8k 对应FFT的k值: [129, 256]  ->>>> 2^6 + 1 , 2^7
        d = 0.064对应的频段: 8k-16k 对应FFT的k值: [257, 512] ->>>> 2^7 + 1 , 2^8
        '''
    def band_filter(self): # 该函数的目的是，对不同的mic进行滤波，使其只负责对应频点的处理。
    ...
    def angle_filter(self, decay_angle = None):
        ...
        k_value, f_unit = self.band_filter() # [512, 256, 128, 64],   32000//2 /512
    ...
    def angleFilter_doa(self, decay_angle = None, bf_method = "cbf"):
        ...
        # 确定搜索角时的P矩阵
        P = np.zeros((513, 49, 31, len(self.angle))) # (k, rou_range.shape, theta.shape)
        ...
        for i in range(len(self.k_value)):
            ...
            for k in range(k_range[0], k_range[1]+1, 1):
                ...
                if phase_angle_array.shape[0] == 0:
                    ...
                    rou_range = np.arange(0, (2.8 - 0) * np.tan(60/180 * np.pi), 0.10)
